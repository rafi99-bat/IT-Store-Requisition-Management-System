...
import fButton.shadow.ShadowRenderer;
import java.awt.Component;
import main.ProductsForm;

/**
 *
 * @author RAVEN
 */
public class FloatingButtonUI extends LayerUI<Component> {
    ...
    public FloatingButtonUI() {
        image = new ImageIcon(getClass().getResource("/fButton/floating/plus.png")).getImage();
    }
    ...
    @Override
    protected void processMouseEvent(MouseEvent me, JLayer<? extends Component> jlayer) {
        if (mouseHovered) {
            me.consume();
        }
        if (SwingUtilities.isLeftMouseButton(me)) {
            if (me.getID() == MouseEvent.MOUSE_PRESSED) {
                ...
                jlayer.repaint(shape.getBounds());
                if (mouseHovered) {
                    ProductsForm p = (ProductsForm) jlayer.getView();
                    p.actionButton();
                }
            }
        }
     ...
     */
    @Override
    protected void processMouseMotionEvent(MouseEvent me, JLayer<? extends Component> jlayer) {
